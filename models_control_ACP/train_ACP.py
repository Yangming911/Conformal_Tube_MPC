...
Inputs:
- p_ped_seq (past pedestrian positions): shape [B, T, 2]
...
from models_control.model_def import ACPCausalPedestrianPredictor, compute_sequence_loss



class SequenceDataset(Dataset):
    def __init__(self, past_ped_seq: np.ndarray, p_ped_seq: np.ndarray):
        assert past_ped_seq.ndim == 3 and past_ped_seq.shape[-1] == 2
        assert p_ped_seq.ndim == 3 and p_ped_seq.shape[-1] == 2
        self.past_ped_seq = torch.from_numpy(past_ped_seq).float()


        self.p_ped_seq = torch.from_numpy(p_ped_seq).float()

    def __len__(self) -> int:
        return self.past_ped_seq.shape[0]

    def __getitem__(self, idx: int):
        return self.past_ped_seq[idx], self.p_ped_seq[idx]


def load_from_csv(csv_path: str, T: int) -> Tuple[np.ndarray, np.ndarray]:
    df = pd.read_csv(csv_path)
    ped_seq_cols = []
    past_ped_seq_cols = []
    for t in range(1, T + 1):
        ped_seq_cols += [f"p_ped{t+10}_x", f"p_ped{t+10}_y"]
        past_ped_seq_cols += [f"p_ped{t}_x", f"p_ped{t}_y"]
    ped_seq = df[ped_seq_cols].to_numpy(dtype=np.float32).reshape(-1, T, 2)
    past_ped_seq = df[past_ped_seq_cols].to_numpy(dtype=np.float32).reshape(-1, T, 2)
    return past_ped_seq, ped_seq
...
def evaluate_model(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for past_p_seq, p_seq in loader:
            ...
            pred = model(past_p_seq)
            loss = mse_loss(pred, p_seq)
            bs = past_p_seq.size(0)
            sum_mse += loss.item() * bs
            total += bs
    avg_mse = sum_mse / max(total, 1)

    # additionally compute per-step MSE for diagnostics
    # gather all preds/targets (small additional pass to avoid storing all)
    per_step_sums = None
    per_step_counts = 0
    with torch.no_grad():
        for past_p_seq, p_seq in loader:
            ...
            p_seq = p_seq.to(device)
            pred = model(past_p_seq)
            err2 = (pred - p_seq) ** 2  # [B,T,2]
            step_mse = err2.mean(dim=2).mean(dim=0)  # [T]
...
def main():
    ...
    past_ped_seq, p_seq = load_from_csv(args.data, args.T)

    N = past_ped_seq.shape[0]
    train_idx, val_idx, test_idx = split_train_val_test(N)

    train_ds = SequenceDataset(past_ped_seq[train_idx], p_seq[train_idx])
    val_ds = SequenceDataset(past_ped_seq[val_idx], p_seq[val_idx])
    test_ds = SequenceDataset(past_ped_seq[test_idx], p_seq[test_idx])
    ...
    model = ACPCausalPedestrianPredictor(p_dim=2, hidden_dim=args.hidden_dim, num_layers=args.layers, dropout=args.dropout)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        model.train()
        epoch_losses = []
        for past_pseq_b, pseq_b in train_loader:
            past_pseq_b = past_pseq_b.to(device)
            pseq_b = pseq_b.to(device)

            pred = model(past_pseq_b)
