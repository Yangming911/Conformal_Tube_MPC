...
def mpc_control(x0, y0, T=10, N=100, d_safe=0.5):
    """
    ...
    if is_tube_safe(forward_car(x0, np.full(T, 15.0)), forward_ped(y0, np.full(T, 15.0), cp_alpha=0.85), d_safe):
        return 15.0
    ...
    for _ in range(N):
        ...
        y_tube = forward_ped(y0, x_tube, cp_alpha=0.85)          # List of sets (e.g., box or sample cloud)
