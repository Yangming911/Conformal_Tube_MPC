...
def update_car_position(car_position, car_v):
    car_position += car_v
    if car_position > 800:
        car_position = -40
        car_v = np.random.uniform(low=1, high=15)
    return car_position, car_v

def update_walker_position(car_v, walker_x_position, walker_y_position):
    if 200 <= walker_y_position < 450:
        walker_v_x, walker_v_y = walker_logic(car_v, walker_y_position)
        walker_y_position += walker_v_y
        walker_x_position += walker_v_x
    else:
