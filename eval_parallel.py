...
from mpc.tubempc_controller_parallel import mpc_control_parallel
...
def calculate_collision_probability(T=10, sample_num=1000, mode='random'):
    collision_count = 0
    average_speed = 0
    ...
    for _ in tqdm(range(sample_num), desc=f"Simulating {mode} T={T}"):
        ...
        for step in range(total_time_steps):
            ...
            if mode == 'random':
                ...
                car_speed = mpc_control_parallel(car_x, np.array([walker_x, walker_y]), T, N, d_safe)
...
def main(args):
    ...
    if args.mode in ['random', 'tubempc', 'vanillampc']:
        ...
        T_values = [1, 10, 20]
        log_file = "mpc_batch_results_parallel.log"
